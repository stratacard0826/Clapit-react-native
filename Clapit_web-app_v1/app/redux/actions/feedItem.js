...
export function createItemReaction(post, accountId, mediaId, comment) {
  ...
  return (dispatch) => {
    ...
    doFetch(`posts/${itemId}/comments`, {
     ...
     /*  no need to subscribe to my own post, because I am already getting notified */
     /*  if(accountId !== post.accountId)
        subPostCommentTopic(itemId);  */
      dispatch({
...
export function deleteReaction(reaction, byPostOwner) {
  ...
  const timeout = setTimeout(() => {
    reloadUserPosts(userId)(dispatch); // eslint-disable-line no-undef
  }, 1500);
  return (dispatch) => {
