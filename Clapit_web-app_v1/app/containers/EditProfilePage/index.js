...
export class EditProfilePage extends React.Component { // eslint-disable-line react/prefer-stateless-function
    ...
    if (uploadedImageUrl !== null) {
      uploadToCloudinary(uploadedImageUrl.type, uploadedImageUrl, (err, res) => {
        if (err || res.status !== 200) {
          this.setState({ modalUserName: true, modalMessage: 'Error upload profile image' });
          setTimeout(() => {
            self.setState({ modalUserName: false });
          }, 3000);
          return false;
        }
        const { url } = res.body;
        createMedia(url)
            .then((data) => {
              if (data.error) {
                this.setState({ modalUserName: true, modalMessage: 'Error create madia profile image' });
                setTimeout(() => {
                  self.setState({ modalUserName: false });
                }, 3000);
                return false;
  ...
  handleCloudinaryCover() {
    ...
    if (uploadedCoverImageUrl !== null) {
      uploadToCloudinary(uploadedCoverImageUrl.type, uploadedCoverImageUrl, (err, res) => {
        if (err || res.status !== 200) {
          this.setState({ modalUserName: true, modalMessage: 'Error upload cover image or video' });
          setTimeout(() => {
            self.setState({ modalUserName: false });
          }, 3000);
          return false;
        ...
        createMedia(url)
            .then((data) => {
              if (data.error) {
                this.setState({ modalUserName: true, modalMessage: 'Error create madia cover image or video' });
                setTimeout(() => {
                  self.setState({ modalUserName: false });
                }, 3000);
                return false;
  ...
  render() {
    ...
    const actionsModalUserName = [
      <FlatButton
        label="Ok"
        primary
        onTouchTap={this.handleModalUserNameClose}
      />,
    ];
    return (
      <div style={{ marginTop: 50 }}>
        {
          clapitAccountData && clapitAccountData.accessToken ?
            <div>
              ...
              <PageContainer>
                <Dialog
                  actions={actionsModalUserName}
                  modal={false}
                  open={this.state.modalUserName}
                  onRequestClose={this.handleModalUserNameClose}
                >
  ...
  _onPressToSave() {
    ...
    if (_.trim(username).length < 2) {
      this.setState({ modalUserName: true, modalMessage: 'Please type a username with at least 2 letters' });
      return false;
    }
    this.setState({
      showLoading: true,
