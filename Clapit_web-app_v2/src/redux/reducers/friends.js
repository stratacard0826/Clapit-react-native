...
export function friends(state = initialState, action = {}) {
  ...
  switch (type) {
    ...
    case FETCH_FRIENDS_DATA_SUCCESS: {
      ...
      postsItems.forEach((item) => newItemsById[createItemId(item)] = item); // eslint-disable-line no-return-assign
    ...
    case SEARCH_FRIENDS_SUCCESS: {
      // set following flag?
      const newFriends = payload.results.map((item) => {
        item.following = (item.friendship === 1); // eslint-disable-line no-param-reassign
        return item;
      });
      // pagination - collect previous results, otherwise new results would override existing results
      const friends = (payload.searchTerm === state.searchTerm && payload.page > 0) ? // eslint-disable-line no-shadow
    ...
    case SEARCH_HASH_TAGS_SUCCESS: {
      const { page, searchTerm, results } = payload;
